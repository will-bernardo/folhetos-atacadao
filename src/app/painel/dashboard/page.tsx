...
export default function DashboardPage() {
  ...
  const fetchConfig = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setHeaderImage(data.header?.imagem || '');
      }
    } catch {
    }
  };
  ...
  const handleSaveHeader = async () => {
    ...
    try {
      const res = await fetch('/api/config', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ header: { imagem: headerImage } }),
      });
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {activeTab === 'config' && (
          <div className="bg-white rounded-xl shadow-md p-6">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">Configurações do Site</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">URL da Imagem do Header</label>
                <input
                  type="url"
                  value={headerImage}
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Cole a URL da imagem hospedada no R2. Deixe vazio para usar o header padrão.
                </p>
              </div>
              {headerImage && (
                <div>
